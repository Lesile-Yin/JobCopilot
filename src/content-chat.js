...
(function () {
  ...
  function highlight(el, color) {
    try {
      ...
      el.style.outline = '3px solid ' + (color || '#00a0e9');
      el.style.transition = 'outline 0.2s';
      setTimeout(() => { el.style.outline = orig; }, 2500);
    } catch (e) {}
  }
  function scrollToEl(el) { if (el) { el.scrollIntoView({ block: 'center', behavior: 'smooth' }); } }
  ...
  async function sendText(greeting) {
    ...
    scrollToEl(input); highlight(input, '#00a0e9');
    input.focus();
    await sleep(400);
    // 多种注入方式依次尝试，确保文字真正填入
    let gotText = false;
    ...
    if (editable) {
      ...
      // textarea 用原生 setter + React 兼容写法
      const setter = Object.getOwnPropertyDescriptor(window.HTMLTextAreaElement.prototype, 'value');
      if (setter) setter.set.call(input, greeting);
      ...
      gotText = !!inputText(input).trim();
      if (!gotText) {
        input.focus(); document.execCommand('insertText', false, greeting);
        await sleep(200);
        gotText = !!inputText(input).trim();
      }
    }
    await sleep(600);
    if (!inputText(input).trim()) return { ok: false, err: '文字未填入输入框｜候选：' + dumpInputs() };
